...
SERVER_CONFIG_FILE=${REALITYCHAIN_SERVER_CONFIG:-$REALITYCHAIN_HOME/xray-server.json}
...
import_env_to_settings() {
	...
	setting_set_if_missing rch_srv_listen "${SERVER_LISTEN:-0.0.0.0}"
	setting_set_if_missing rch_srv_port "${SERVER_LISTEN_PORT:-${SERVER_PORT:-443}}"
	setting_set_if_missing rch_reality_private_key "${REALITY_PRIVATE_KEY:-}"
	setting_set_if_missing rch_srv_config "$SERVER_CONFIG_FILE"
...
sync_settings_to_env() {
	...
	server_listen=$(clean_value "$(setting_get_or rch_srv_listen "${SERVER_LISTEN:-0.0.0.0}")")
	server_listen_port=$(clean_value "$(setting_get_or rch_srv_port "${SERVER_LISTEN_PORT:-${SERVER_PORT:-443}}")")
	reality_private_key=$(clean_value "$(setting_get_or rch_reality_private_key "${REALITY_PRIVATE_KEY:-}")")
	...
	tmp_file=$ENV_FILE.$$
	{
		...
		write_env_line SERVER_LISTEN "$server_listen"
		write_env_line SERVER_LISTEN_PORT "$server_listen_port"
...
update_server_status() {
	server_status=$1
	load_helper
	am_settings_set rch_srv_status "$server_status"
	am_settings_set rch_srv_config "$(setting_get_or rch_srv_config "$SERVER_CONFIG_FILE")"
	am_settings_set rch_updated_at "$(date '+%Y-%m-%d %H:%M:%S %Z' 2>/dev/null || date)"
}
...
render_server_config() {
	sync_settings_to_env
	load_helper
	output_file=$(setting_get_or rch_srv_config "$SERVER_CONFIG_FILE")
	[ -n "$output_file" ] || output_file=$SERVER_CONFIG_FILE

	if "$REALITYCHAIN_HOME/realitychainctl" render-server "$ENV_FILE" "$output_file" >/tmp/realitychain-webui-action.log 2>&1; then
		update_server_status rendered
	else
		update_server_status failed
		die "failed to render server config; see /tmp/realitychain-webui-action.log"
	fi
}
