            ...
            if let estimate = model.selectedEstimate {
                ...
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    HStack(spacing: 10) {
                        ...
                        Button("Export JSON") {
                            if let url = model.exportScanReport() {
                                NSWorkspace.shared.activateFileViewerSelecting([url])
                                model.statusMessage = "Exported scan report to \(url.lastPathComponent)"
                            }
                        }
                        .font(.custom("Avenir Next Medium", size: 12))
                        .buttonStyle(.plain)
                        .foregroundStyle(Theme.accent)
                        .disabled(model.accessPoints.isEmpty && model.estimates.isEmpty)
                    }
                }
                .padding(16)
                .background(Theme.surface)
